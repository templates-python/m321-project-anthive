...
def register():
    ...
    :return: None
    """
    hive01 = {'action': 'register', 'ip': ANTHILL_HOST, 'port': ANTHILL_PORT, 'type': 'hive'}
    hive02 = {'action': 'register', 'ip': 696969, 'port': ANTHILL_PORT, 'type': 'hive'}
    send_request(hive01)
    send_request(hive02)
...
def process_response(action, message):
    print(f"MESSAGE FROM DISCOVERY SERVER GOT: {message.response}")
...
def game():
    ...
    lsock.listen()
    print(f'Hive: listening on {(ANTHILL_HOST, ANTHILL_PORT)}')
            ...
            for key, mask in events:
                if key.data is None:
                    ...
                    try:
                        ...
                        print(
                            f'Hive: Error: Exception for {message.ipaddr}:\n'
                            f'{traceback.format_exc()}'
                        )
                        message.close()
    except KeyboardInterrupt:
        print('Hive: Caught keyboard interrupt, exiting')
    finally:
        sel.close()
